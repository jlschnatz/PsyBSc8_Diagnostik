...
author    : "Luca Schnatz & Arieja Farugie"
date: "`r Sys.Date()`"

bibliography: efa_ref.bib
csl: apa.csl
code-overflow: wrap
code-block-bg: true
...
library(tidyverse)
library(here)
data_gis_final <- read_csv(here("src/data/data-gis-final.csv"))
...
```{r}
library(sjPlot)
sjt.itemanalysis(df = data_gis_final, factor.groups.titles = NULL)
```
...
tab_fa(
 ...
 psych::fa(data_gis_final, nfactors = 1) %>%
  chuck("loadings") %>%
  as.data.frame.table() %>%
   select(Item = Var1, Loading = Freq) %>%
   tab_df()
 
 
 fa(data_gis_final, nfactors = 1, fm = "pa")$loadings
...
data_gis_fa1 %>% 
  ...
  stat_theodensity(aes(y = after_stat(count), color = factor), color = "darkblue") +
  facetted_pos_scales(
    x = list(
